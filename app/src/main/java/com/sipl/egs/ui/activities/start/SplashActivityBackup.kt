        ...
        binding.progressBar.visibility = View.VISIBLE
           CoroutineScope(Dispatchers.IO).launch {
               ...
               if(!mySharedPref.getAllAreaEntries())
                   ...
                   {
                      val areaEnteriesJob=async {  val items = readJsonFromAssets(this@SplashActivityBackup, "address.json")
              ...
              /* fetchMastersFromServer()
               ...
               withContext(Dispatchers.Main) {
                   ...
                   if(mySharedPref.getIsLoggedIn() && mySharedPref.getRoleId()==3){
                       ...
                       binding.progressBar.visibility = View.GONE
                       val intent= Intent(this@SplashActivityBackup, OfficerMainActivity::class.java)
                       intent.flags = Intent.FLAG_ACTIVITY_CLEAR_TOP or Intent.FLAG_ACTIVITY_CLEAR_TASK or Intent.FLAG_ACTIVITY_NEW_TASK
                       startActivity(intent)
                       finish()
                   } else{
                ...
                override fun onResponse(
                    ...
                    if(response.isSuccessful){
                        if(response.body()?.status.equals("success")) {
                            ...
                            Toast.makeText(this@SplashActivityBackup, resources.getString(R.string.no_records_found), Toast.LENGTH_SHORT)
                                .show()
                        }
                    } else{
                        Log.d("mytag","fetchMastersFromServer:Response unsuccessful")
                        Toast.makeText(this@SplashActivityBackup, resources.getString(R.string.response_unsuccessfull), Toast.LENGTH_SHORT).show()
                    }

                }
                override fun onFailure(call: Call<MastersModel>, t: Throwable) {
                    Log.d("mytag","fetchMastersFromServer:onFailure ${t.message}")
                    Toast.makeText(this@SplashActivityBackup, resources.getString(R.string.error_occured_during_api_call), Toast.LENGTH_SHORT).show()
                }
            })
        } catch (e: Exception) {
