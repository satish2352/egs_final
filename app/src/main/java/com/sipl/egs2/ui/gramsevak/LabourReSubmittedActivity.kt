...
import com.sipl.egs2.databinding.ActivityLabourReSubmittedBinding
...
class LabourReSubmittedActivity : AppCompatActivity(),
    ...
    private fun getDataFromServer(currentPage:String) {
        try {
            dialog.show()
            val call = apiService.getLabourListReSubmittedForGramSevak(pageNumber = currentPage)
                ...
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        if (response.body()?.status.equals("true")) {
                            ...
                            val pageAdapter=MyPaginationAdapter(response.body()?.totalPages!!,response.body()?.page_no_to_hilight.toString(),this@LabourReSubmittedActivity)
    ...
    override fun onPageNumberClicked(pageNumber: Int) {
        if (isInternetAvailable) {
            ...
            Toast.makeText(this@LabourReSubmittedActivity,resources.getString(R.string.internet_is_not_available_please_check),Toast.LENGTH_SHORT).show()
        }
